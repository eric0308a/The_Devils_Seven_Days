...
　　var DrillUp = DrillUp || {}; 
	...
	DrillUp.drill_APEx_initCondition = function( dataFrom ) {
		...
		data['trigger_p_state_default'] = String( dataFrom["前视图-是否恢复为默认集合"] || "false") == "true";
		...
		data['trigger_b_state_default'] = String( dataFrom["背景图-是否恢复为默认集合"] || "false") == "true";
...
Drill_APEx_Sprite.prototype.drill_APEx_updateCondition = function() {
	...
	for(var i=this._drill_conditions.length-1; i >= 0 ; i--){
		...
		if( condition['switch_enable'] == true ){	
			if( $gameSwitch.value( condition['switch_id'] ) == true ){
				//（不操作）
			}else{
				continue; //（开了条件，却不满足时，跳过）
			}
		...
		if( condition['switch2_enable'] == true ){	
			if( $gameSwitch.value( condition['switch2_id'] ) == true ){
				//（不操作）
			}else{
				continue; //（开了条件，却不满足时，跳过）
			}
		...
		if( condition['variable_enable'] == true ){	
			if( condition['variable_enable'] == "大于等于" ){
				if( $gameSwitch.value( condition['variable_id'] ) >= condition['variable_value'] ){
					//（不操作）
				}else{
					continue; //（开了条件，却不满足时，跳过）
				}
			}
			if( condition['variable_enable'] == "小于等于" ){
				if( $gameSwitch.value( condition['variable_id'] ) <= condition['variable_value'] ){  }else{ continue; }
			}
			if( condition['variable_enable'] == "大于" ){
				if( $gameSwitch.value( condition['variable_id'] ) > condition['variable_value'] ){  }else{ continue; }
			}
			if( condition['variable_enable'] == "小于" ){
				if( $gameSwitch.value( condition['variable_id'] ) < condition['variable_value'] ){  }else{ continue; }
			}
			if( condition['variable_enable'] == "等于" ){
				if( $gameSwitch.value( condition['variable_id'] ) == condition['variable_value'] ){  }else{ continue; }
			}
		}
