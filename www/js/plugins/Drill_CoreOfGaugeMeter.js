...
　　var DrillUp = DrillUp || {}; 
	...
	DrillUp.drill_COGM_initStyle = function( dataFrom ) {
		...
		data['shorten_speed'] = Math.abs( Number( dataFrom["缩短速度"] || 2.5) );
...
// **			  2.【temp_data配置参数】都在drill_initData中，其他的都为私有参数。
// **		 		你可以先取【DrillUp.g_COGM_list样式数据】再赋值各个额外属性，也可以【直接new】全参数自己建立控制。
// **		 		其中，"level_max"段上限 由于其特殊性，是会贯穿于所有子插件的。
// **			  3.需要实时调用函数.drill_reflashValue(value)改变参数条的值。
// **			  4.值减少时，凹槽条、弹出条会产生效果，值增加不会。
//=============================================================================
//==============================
// * 参数条 - 定义
//==============================
...
Drill_COGM_MeterSprite.prototype.drill_initData = function() {
	...
	if( data['shorten_speed'] == undefined ){ data['shorten_speed'] = 5.0 };						//分段条 - 缩短速度
...
Drill_COGM_MeterSprite.prototype.drill_updateLevelValue = function() {
	...
	// > 缩短效果 - 瞬间/弹性缩短
	if( data['shorten_mode'] == "瞬间缩短" || data['shorten_speed'] == 0 ){
		...
		if(this._drill_level_cur_value < this._drill_new_value){
			var diff = Math.max( ( this._drill_new_value - this._drill_level_cur_value)/data['shorten_speed'] , 1 );
		...
		if(this._drill_level_cur_value > this._drill_new_value){
			var diff = Math.max( ( this._drill_level_cur_value - this._drill_new_value)/data['shorten_speed'] , 1 );
			this._drill_level_cur_value -= diff;
			if( this._drill_level_cur_value < this._drill_new_value ){
				this._drill_level_cur_value = this._drill_new_value;
			}
...
Drill_COGM_SpringSprite.prototype.drill_resetData = function( data, parent_bitmap_obj ) {
	...
	// > 私有对象初始化
	this._drill_life = data['ballistics']['movementTime'];		//持续时间
...
Drill_COGM_SpringSprite.prototype.drill_updateSprite = function() {
	...
	if( time <= 0 ){ time = 0; }
	if( time >= this['_drill_COBa_x'].length ){
	...
	// > 逐渐透明
	this.opacity = 255 - 255 * time/this._drill_life;
}
