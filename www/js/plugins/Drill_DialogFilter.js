	...
	if (command === '>对话框滤镜') {	//>对话框滤镜 : 全部 : 纯色滤镜 : 纯黑 : 155 : 60
		if(args.length == 8 || args.length == 10){
			...
			for(var j=0; j<tar_s.length; j++){
				if( type == "纯色滤镜" ){
					tar_s[j].setPureLinear = [temp1,temp2,temp3];
				}
				if( type == "着色滤镜" ){
					tar_s[j].setColorLinear = [temp1,temp2,temp3];
				}
				if( type == "填充滤镜" ){
					tar_s[j].setFillLinear = [temp1,temp2,temp3];
				}
				if( type == "模糊滤镜" ){
					tar_s[j].setBlurLinear = [Number(temp1),temp2];
				}
				if( type == "噪点滤镜" ){
					tar_s[j].setNoiseLinear = [Number(temp1),temp2];
				}
				if( type == "波动纯色滤镜" ){
					tar_s[j].setPureWave = [temp1,temp2,temp3];
				}
				if( type == "波动着色滤镜" ){
					tar_s[j].setColorWave = [temp1,temp2,temp3];
				}
				if( type == "波动填充滤镜" ){
					tar_s[j].setFillWave = [temp1,temp2,temp3];
				}
				if( type == "波动模糊滤镜" ){
					tar_s[j].setBlurWave = [Number(temp1),temp2];
				}
				if( type == "波动噪点滤镜" ){
					tar_s[j].setNoiseWave = [Number(temp1),temp2];
				}
			}
		}
	}
