 ...
 * 设计：
 *   (1.插件一般用于 地图界面 或者 战斗界面 的简易教程图示。
 *   (2.插件的功能完全可以用纯事件做：
 *      先控制显示图片，再播放一个隐形对话框（背景透明+空内容），效果
 *      是一模一样的。
 *      不过，纯事件有一个小瑕疵，你能看见对话框的小箭头，而插件没有。
 *
 * -----------------------------------------------------------------------------
 * ----关联文件
...
Window_Message.prototype.initialize = function() {
	...
	this._drill_orgX = 0;		//原位置x
	this._drill_orgY = 0;		//原位置y
	
	this._drill_DSS = {};				//
	this._drill_DSS.x = 0;				//变动x
	this._drill_DSS.y = 0;				//变动y
...
Window_Message.prototype.update = function() {
	...
	this.drill_DSS_updatePosition();		//固定帧初始值
...
Window_Message.prototype.drill_DSS_updateEffect = function() {
	...
	//this.rotation += this._drill_DSS.rotation;	// 旋转
	//this.scale.x += this._drill_DSS.scale_x;		// 缩放x
	//this.scale.y += this._drill_DSS.scale_y;		// 缩放y
	//this.skew.x += this._drill_DSS.skew_x;		// 斜切x
	//this.skew.y += this._drill_DSS.skew_y;		// 斜切y
}
...
// * 对话图 - 创建
//==============================
var _drill_DSS_Scene_createAllWindows = Scene_Map.prototype.createAllWindows;
Scene_Map.prototype.createAllWindows = function() {
	_drill_DSS_Scene_createAllWindows.call(this);
