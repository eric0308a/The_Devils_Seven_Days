 ...
 * 1.插件的作用域：战斗界面。
 *   角色、敌人都可以添加滤镜效果。
 * 2.单位滤镜效果在 战斗 > 敌群 中有示例说明。
 * 3.纯色滤镜、着色滤镜……等 相互独立，且效果可以相互叠加。
 *   添加滤镜的先后顺序不同，能产生不同的叠加效果。
 * 4.波动纯色滤镜 与 纯色滤镜 是同一个滤镜，只是变化方式不同。
 *   二者指令会相互覆盖。
 * 5.使用滤镜时，最好先设置0（给一个关闭滤镜过程），再切换。避免瞬间切换的不自然。
 * 6.由于 mog角色图像 插件难以支持滤镜效果，所以未作扩展。
 * 7.v1.1版本之前的关键字是">敌人滤镜"，这里一样有效，只是">单位滤镜"更规范。
 * 
 * -----------------------------------------------------------------------------
 * ----激活条件
 * 你需要通过插件指令设置滤镜：
...
//			  本质的rmmv，是物体控制数据，贴图直接变化即可，而滤镜直接破坏了规则，控制数据和变化数据直接交织。
//			  导致了物体传数据给贴图成了麻烦。
					...
					this.iterateEnemyIndex(-1,function(enemy) {
						if (enemy.isAlive()) {
							if( type == "纯色滤镜" ){
								...
								enemy._drill_EFi.setPureLinear = [temp1,temp2,temp3];
							}
							if( type == "着色滤镜" ){
								enemy._drill_EFi.openFilter = true;
								enemy._drill_EFi.setColorLinear = [temp1,temp2,temp3];
							}
							if( type == "填充滤镜" ){
								enemy._drill_EFi.openFilter = true;
								enemy._drill_EFi.setFillLinear = [temp1,temp2,temp3];
							}
							if( type == "模糊滤镜" ){
								enemy._drill_EFi.openFilter = true;
								enemy._drill_EFi.setBlurLinear = [Number(temp1),temp2];
							}
							if( type == "噪点滤镜" ){
								enemy._drill_EFi.openFilter = true;
								enemy._drill_EFi.setNoiseLinear = [Number(temp1),temp2];
							}
							if( type == "波动纯色滤镜" ){
								enemy._drill_EFi.openFilter = true;
								enemy._drill_EFi.setPureWave = [temp1,temp2,temp3];
							}
							if( type == "波动着色滤镜" ){
								enemy._drill_EFi.openFilter = true;
								enemy._drill_EFi.setColorWave = [temp1,temp2,temp3];
							}
							if( type == "波动填充滤镜" ){
								enemy._drill_EFi.openFilter = true;
								enemy._drill_EFi.setFillWave = [temp1,temp2,temp3];
							}
							if( type == "波动模糊滤镜" ){
								enemy._drill_EFi.openFilter = true;
								enemy._drill_EFi.setBlurWave = [Number(temp1),temp2];
							}
							if( type == "波动噪点滤镜" ){
								enemy._drill_EFi.openFilter = true;
								enemy._drill_EFi.setNoiseWave = [Number(temp1),temp2];
							}
						}
					}.bind(this));
				}else{
					...
					this.iterateActorId(0,function(actor) {
						if (actor.isAlive()) {
							if( type == "纯色滤镜" ){
								...
								actor._drill_EFi.setPureLinear = [temp1,temp2,temp3];
							}
							if( type == "着色滤镜" ){
								actor._drill_EFi.openFilter = true;
								actor._drill_EFi.setColorLinear = [temp1,temp2,temp3];
							}
							if( type == "填充滤镜" ){
								actor._drill_EFi.openFilter = true;
								actor._drill_EFi.setFillLinear = [temp1,temp2,temp3];
							}
							if( type == "模糊滤镜" ){
								actor._drill_EFi.openFilter = true;
								actor._drill_EFi.setBlurLinear = [Number(temp1),temp2];
							}
							if( type == "噪点滤镜" ){
								actor._drill_EFi.openFilter = true;
								actor._drill_EFi.setNoiseLinear = [Number(temp1),temp2];
							}
							if( type == "波动纯色滤镜" ){
								actor._drill_EFi.openFilter = true;
								actor._drill_EFi.setPureWave = [temp1,temp2,temp3];
							}
							if( type == "波动着色滤镜" ){
								actor._drill_EFi.openFilter = true;
								actor._drill_EFi.setColorWave = [temp1,temp2,temp3];
							}
							if( type == "波动填充滤镜" ){
								actor._drill_EFi.openFilter = true;
								actor._drill_EFi.setFillWave = [temp1,temp2,temp3];
							}
							if( type == "波动模糊滤镜" ){
								actor._drill_EFi.openFilter = true;
								actor._drill_EFi.setBlurWave = [Number(temp1),temp2];
							}
							if( type == "波动噪点滤镜" ){
								actor._drill_EFi.openFilter = true;
								actor._drill_EFi.setNoiseWave = [Number(temp1),temp2];
							}
						}
					}.bind(this));
				}else{
