...
/*:
 * @plugindesc [v1.7]        物体触发 - 固定区域 & 玩家接近 & 条件触发
 * @author Drill_up
 * 
 ...
 * 设计：
 *   (1.常用设计是玩家接近事件后，事件自己快速做出反应，比如逃跑/战斗等。
 *   (2.你也可以设计成某种接近自动触发的开关。
 * 
 * -----------------------------------------------------------------------------
 * ----激活条件 - 被触发
 ...
 *   设置后，如果玩家注释的关键字与被动的对应上，将会触发开启独立开关。
 ...
 * （注意，冒号左右有一个空格）
 *
 * 插件指令：>玩家接近主动触发 : 关闭触发 : 2
 * 插件指令：>玩家接近主动触发 : 开启触发 : 2
 *
...
　　var DrillUp = DrillUp || {}; 
	...
	for (var i = 0; i < DrillUp.g_EAT_area_length; i++) {
		if( DrillUp.parameters['玩家触发-' + String(i+1) ] != "" ){
			...
			DrillUp.g_EAT_area[i]['self_id'] = Number(DrillUp.g_EAT_area[i]["自定义区域编号"] || 1)  - 1;
			var self_id = DrillUp.g_EAT_area[i]['self_id'];
			if( self_id > 0 ){
...
Game_Map.prototype.drill_EAT_updateEventsSort = function() {
	// >同步事件数组（变化了才同步）
	if( this._drill_EAT_xy_events == undefined || this._drill_EAT_xy_events.length != this._events.length ){
		this._drill_EAT_xy_events = this._events.slice();
	}
	// >排序（离玩家越远越后，空事件最后）
	this._drill_EAT_xy_events.sort(function(a, b){
		if( !$gameMap ){ return 0 };
		if( !a || a._erased ){ return 1 };
		if( !b || b._erased ){ return -1 };
		var distance_a = $gameMap.distance(a._realX,a._realY,$gamePlayer._realX,$gamePlayer._realY); 
		var distance_b = $gameMap.distance(b._realX,b._realY,$gamePlayer._realX,$gamePlayer._realY); 
...
Game_Map.prototype.drill_EAT_getEventsInMaxRange = function() {
	...
	for(var i=0; i<this._drill_EAT_xy_events.length ; i++){
		var event = this._drill_EAT_xy_events[i];
		if( !event || event._erased ||
			Math.abs(event._realX - $gamePlayer._realX) > DrillUp.g_EAT_area_max_x_range + 1 ||
			Math.abs(event._realY - $gamePlayer._realY) > DrillUp.g_EAT_area_max_y_range + 1
			 ){
			break;
		}
		result.push(event);
