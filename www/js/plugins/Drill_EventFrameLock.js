 ...
 * 锁定帧：
 *   (1.锁定后，无论行走图如何、朝向如何，行走图都为rmmv编辑器中设
 *      置的帧图像。不会被更改。
 *   (2.固定帧与锁定帧的定义不一样，见"关于行走图与图块.docx"。
 ...
 * （注意，冒号左右都有一个空格）
 * 
 * 插件指令：>行走图锁定帧 : 本事件 : 锁定帧动画 : 从左往右 : 8
 * 插件指令：>行走图锁定帧 : 事件[10] : 锁定帧动画 : 从左往右 : 8
 * 插件指令：>行走图锁定帧 : 事件变量[21] : 锁定帧动画 : 从左往右 : 8
 * 插件指令：>行走图锁定帧 : 批量事件[10,11] : 锁定帧动画 : 从左往右 : 8
 * 插件指令：>行走图锁定帧 : 批量事件变量[21,22] : 锁定帧动画 : 从左往右 : 8
 * 
 * 插件指令：>行走图锁定帧 : 本事件 : 锁定帧动画 : 从左往右 : 8
 * 插件指令：>行走图锁定帧 : 本事件 : 锁定帧动画 : 从右往左 : 8
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === '>行走图锁定帧') {
		...
		if(args.length >= 4){
			...
			if(args[7] != undefined){ var temp3 = String(args[7]); }
			var e_ids = null;
			...
			if( e_ids == null && !isNaN(eid) ){ 	//>行走图锁定帧 : 10 : 锁定
				e_ids = [];
				e_ids.push( eid );
			}
