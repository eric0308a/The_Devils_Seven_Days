 ...
 * 
 * 插件指令：>可拾取物生成器 : 音效 : 设置 : 1
 * 插件指令：>可拾取物生成器 : 音效 : 设置随机之一 : 1,2,3,4
 *
...
　　var DrillUp = DrillUp || {}; 
	...
	if( DrillUp.parameters['资源-可拾取物图像'] != undefined ){
		DrillUp.g_EIG_itemPic = JSON.parse(DrillUp.parameters['资源-可拾取物图像']);
	}else{
...
Game_Map.prototype.drill_EIG_generateItemEvent = function( event_data ) {
	...
	if( event_data['item_type'] == "金钱"){
		...
		var random_img = DrillUp.g_EIG_goldPic[index_img];
		var random_dir = 2;
		...
		var random_img = DrillUp.g_EIG_itemPic[index_img];
		var random_dir = 2;
