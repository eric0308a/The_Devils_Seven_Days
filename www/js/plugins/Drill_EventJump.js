...
// * 路线 - 事件默认路线
//==============================
var _drill_EJu_setMoveRoute = Game_Character.prototype.setMoveRoute;
Game_Character.prototype.setMoveRoute = function(moveRoute) {
	moveRoute.list = this.drill_EJu_moveRouteTransform(moveRoute.list);
	_drill_EJu_setMoveRoute.call(this, moveRoute);
};
//==============================
// * 路线 - 函数设置路线
//==============================
var _drill_EJu_forceMoveRoute = Game_Character.prototype.forceMoveRoute;
Game_Character.prototype.forceMoveRoute = function(moveRoute) {
	moveRoute.list = this.drill_EJu_moveRouteTransform(moveRoute.list);
	_drill_EJu_forceMoveRoute.call(this, moveRoute);
};
//==============================
// * 路线 - 修改路线内容
//==============================
Game_Character.prototype.drill_EJu_moveRouteTransform = function(route_list) {
	var r_list = [];
	route_list.forEach(function(r){
		if (r.code === 45) {
			var matched = false;
			var args = r.parameters[0].split(/[:：]/);
			var command = args.shift();
			if( command == ">普通跳跃" ){
				if( args.length == 0 ){
					matched = true;
					var _script = "this.drill_EJu_commonJump();";
					r_list.push({ "code":45,"parameters":[_script] });
				}
				if( args.length == 2 ){
					var temp2 = String(args[0]);
					var temp3 = String(args[1]);
					temp2 = "\""+temp2+"\"";
					temp3 = temp3.replace("距离[","");
					temp3 = temp3.replace("]","");
					if( temp3 == "属性值" ){
						matched = true;
						var _script = "this.drill_EJu_commonJumpCommand( this._drill_EJu_jump['distance'],"+temp2+" );";
						r_list.push({ "code":45,"parameters":[_script] });
					}else{
						matched = true;
						var _script = "this.drill_EJu_commonJumpCommand( "+Number(temp3)+","+temp2+" );";
						r_list.push({ "code":45,"parameters":[_script] });
					}
				}
			}
			if( matched == false ){ r_list.push(r); }
		} else {
			r_list.push(r);
		}
	}.bind(this));
...
Game_CharacterBase.prototype.drill_EJu_canPassJump = function(x, y, cur_cliff) {
    ...
    if ( this.drill_EJu_getCliffHeight(x, y) > cur_cliff ) {//悬崖高度
        return false;
    };
    if (!$gameMap.drill_EJu_isAnyPassable(x, y)) {		//图块可通行状况
        return false;
    };
