 ...
 * 参数条：
 *   (1.你需要先配置参数条样式，才能将样式id对应到指定的 槽样式 中。
 *      参数条样式配置在 参数条核心 中配置。
 *   (2.如果你有不同想法，参数条可以设置多段、游标、凹槽条 等。
 * 参数数字：
 *   (1.你需要先配置参数数字样式，才能将样式id对应到指定的 槽样式 中。
 *      参数数字样式配置在 参数数字核心 中配置。
 *   (2."x19"或"+100/100"等数字组合写法，都在核心中配置样式，该插件只
 *      关联样式id。
 * 名称：
 *   (1.注意，名称不能支持特殊字符，也不能支持高级颜色。
 * 设计：
...
　　var DrillUp = DrillUp || {}; 
	...
	for (var i = 0; i < DrillUp.g_GFV_styleList_length; i++) {
		if( DrillUp.parameters["固定框样式-" + String(i+1) ] != "" ){
			DrillUp.g_GFV_styleList[i] = JSON.parse(DrillUp.parameters["固定框样式-" + String(i+1) ]);
			DrillUp.g_GFV_styleList[i] = DrillUp.drill_GFV_initStyle( DrillUp.g_GFV_styleList[i] );
			//alert(JSON.stringify(DrillUp.g_GFV_styleList[i]));
		}else{
	...
	for (var i = 0; i < DrillUp.g_GFV_bind_length; i++) {
		if( DrillUp.parameters["变量框设置-" + String(i+1) ] != "" ){
			...
			DrillUp.g_GFV_bind[i] = DrillUp.drill_GFV_initBind( DrillUp.g_GFV_bind[i] );
		}else{
