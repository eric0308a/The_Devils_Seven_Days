 ...
 *   作用于物品、技能的名称。
 * 2.该插件与 UI-物品+技能文本颜色 相互独立，但是为了使得效果
 *   叠加，则必须放其前面。
 * 3.该插件不支持 技能块元素 的滤镜变色。
 * 
 * -----------------------------------------------------------------------------
 * ----激活条件：
		...
		if(commands != "" && commands != [] ){
			...
			if(args.length >= 2 && args.length <= 4){
				...
				if( type == "纯色滤镜" ){		//<滤镜:纯色滤镜:纯蓝:55>
					...
					f_data.setPureLinear = [String(temp1),temp2,0];
				}
				if( type == "着色滤镜" ){
					f_data.openFilter = true;
					f_data.setColorLinear = [String(temp1),temp2,0];
				}
				if( type == "填充滤镜" ){
					f_data.openFilter = true;
					f_data.setFillLinear = [String(temp1),temp2,0];
				}
				...
				if( type == "波动纯色滤镜" ){	//<滤镜:波动纯色滤镜:纯蓝:55:60>
					f_data.openFilter = true;
					f_data.setPureWave = [String(temp1),temp2,temp3];
				}
				if( type == "波动着色滤镜" ){
					f_data.openFilter = true;
					f_data.setColorWave = [String(temp1),temp2,temp3];
				}
				if( type == "波动填充滤镜" ){
					f_data.openFilter = true;
					f_data.setFillWave = [String(temp1),temp2,temp3];
				}
