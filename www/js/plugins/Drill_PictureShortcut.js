 ...
 *   作用于rmmv图片。
 * 预加载资源：
 *   (1.rmmv的图片资源并不是预加载的，如果图片过大，显示时会闪，这是因
 ...
 *   (2.图片的中心锚点是可以修改的，rmmv默认有左上(0,0)和中心(0.5,0.5)
 *      的设置。中心锚点会影响部分动作效果。
 ...
 * @param 预加载资源-1
 * @parent ---预加载资源组 1至20---
 * @desc 将rmmv图片放入预加载资源中，进入游戏后会提前加载，使用时不会出现闪烁情况。
 * @default 
 * @require 1
 * @dir img/pictures/
 * @type file
...
Game_Picture.prototype.drill_PSh_updateCommandExecute = function() {
	for( var i=0; i < this._Drill_PSh_commandTank.length; i++ ){
		...
		if( command["type"] == "relativeElasticMove" || command["type"] == "elasticMove" ){
			...
			var time_last = (command["cur_time"]-1);	//逆向时间
			...
			if( time_last > command["time"] ){ diff_x2 = 0; }
			...
			if( time_last > command["time"] ){ diff_y2 = 0; }
			this._y -= diff_y1 - diff_y2;
		}
		
		command["cur_time"] += 1;
		if( command["cur_time"] >= command["time"]){ command["needRemove"] = true; }
	}
}
//==============================
// * 帧刷新 - 去除指令
...
// * 图片贴图 - 斜切控制
//==============================
var _Drill_PFOE_sp_updateScale = Sprite_Picture.prototype.updateScale;
Sprite_Picture.prototype.updateScale = function() {
	_Drill_PFOE_sp_updateScale.call(this);
    var picture = this.picture();
...
Scene_Map.prototype.createAllWindows = function() {
	_drill_PSh_layer_createAllWindows.call(this);	//rmmv对话框 < 最顶层
...
Scene_Battle.prototype.createAllWindows = function() {
	_drill_PSh_battle_createAllWindows.call(this);	//rmmv对话框 < 最顶层
	if( !this._drill_SenceTopArea ){
		this._drill_SenceTopArea = new Sprite();
		this.addChild(this._drill_SenceTopArea);	
	}
