 ...
 *   (3.窗口之间的流程比较复杂，建议去看看 "关于限量商店界面.docx"。
 *      注意，购买流程 是完全固定的业务逻辑，无法修改。
 * 窗口：
	...
	if (command === '>限量商店') {
		...
		if(args.length == 6){
			...
			if( type == "切换服务员" ){
				...
				$gameSystem._drill_SLS_shopList[ temp1 ]['waitress_id'] = temp2;
			}
		}
		if(args.length == 8){
			var temp1 = String(args[1]);
			...
			if( type == "添加上限" ){
				...
				temp3 = temp3.replace("]","");
				temp3 = Number(temp3);
				var item_shop = $gameSystem._drill_SLS_shopList[ temp1 ]['list'][ temp2 ];
				item_shop['limit_num'] += temp3;
				if( item_shop['limit_num'] < 0 ){
					item_shop['limit_num'] = 0;
				}
			}
		}
	}
...
Game_System.prototype.initialize = function() {	
	...
	this._drill_SLS_shopList = DrillUp.g_SLS_shop_list;		//商店商品
};
