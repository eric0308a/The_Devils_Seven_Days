...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === '>信息面板B') {
		if(args.length == 2){
			...
			if( type == "显示全部" ){
				for( var i = 1; i <= DrillUp.g_SSpB_context_list_length; i++){
					DrillUp.g_SSpB_context_list[i]['enabled'] = true;	//全局存储
					if( !$gameSystem._drill_SSpB_context_list ){ $gameSystem.drill_SSpB_dataInit(); }
					$gameSystem._drill_SSpB_context_list[i]['enabled'] = true;	//正常存储
				}
				DataManager.forceSaveGlobalInfo();
			}
			if( type == "隐藏全部" ){
				for( var i = 1; i <= DrillUp.g_SSpB_context_list_length; i++){
					DrillUp.g_SSpB_context_list[i]['enabled'] = false;	//全局存储
					if( !$gameSystem._drill_SSpB_context_list ){ $gameSystem.drill_SSpB_dataInit(); }
					$gameSystem._drill_SSpB_context_list[i]['enabled'] = false;	//正常存储
				}
				DataManager.forceSaveGlobalInfo();
			}
			if( type == "锁定全部" ){
				for( var i = 1; i <= DrillUp.g_SSpB_context_list_length; i++){
					DrillUp.g_SSpB_context_list[i]['locked'] = true;	//全局存储
					if( !$gameSystem._drill_SSpB_context_list ){ $gameSystem.drill_SSpB_dataInit(); }
					$gameSystem._drill_SSpB_context_list[i]['locked'] = true;	//正常存储
				}
				DataManager.forceSaveGlobalInfo();
			}
			if( type == "解锁全部" ){
				for( var i = 1; i <= DrillUp.g_SSpB_context_list_length; i++){
					DrillUp.g_SSpB_context_list[i]['locked'] = false;	//全局存储
					if( !$gameSystem._drill_SSpB_context_list ){ $gameSystem.drill_SSpB_dataInit(); }
					$gameSystem._drill_SSpB_context_list[i]['locked'] = false;	//正常存储
				}
				DataManager.forceSaveGlobalInfo();
			}
		}
	...
	if (command === '>信息面板B') {
		if(args.length == 4){
			...
			if( type == "显示选项" ){
				DrillUp.g_SSpB_context_list[temp1]['enabled'] = true;	//全局存储
				DataManager.forceSaveGlobalInfo();
				if( !$gameSystem._drill_SSpB_context_list ){ $gameSystem.drill_SSpB_dataInit(); }
				$gameSystem._drill_SSpB_context_list[temp1]['enabled'] = true;	//正常存储
			}
			if( type == "隐藏选项" ){
				DrillUp.g_SSpB_context_list[temp1]['enabled'] = false;	//全局存储
				DataManager.forceSaveGlobalInfo();
				if( !$gameSystem._drill_SSpB_context_list ){ $gameSystem.drill_SSpB_dataInit(); }
				$gameSystem._drill_SSpB_context_list[temp1]['enabled'] = false;	//正常存储
			}
			if( type == "锁定选项" ){
				DrillUp.g_SSpB_context_list[temp1]['locked'] = true;	//全局存储
				DataManager.forceSaveGlobalInfo();
				if( !$gameSystem._drill_SSpB_context_list ){ $gameSystem.drill_SSpB_dataInit(); }
				$gameSystem._drill_SSpB_context_list[temp1]['locked'] = true;	//正常存储
			}
			if( type == "解锁选项" ){
				DrillUp.g_SSpB_context_list[temp1]['locked'] = false;	//全局存储
				DataManager.forceSaveGlobalInfo();
				if( !$gameSystem._drill_SSpB_context_list ){ $gameSystem.drill_SSpB_dataInit(); }
				$gameSystem._drill_SSpB_context_list[temp1]['locked'] = false;	//正常存储
			}
			if( type == "选中页" ){
				var temp = temp1 -1;
				if( temp < 0 ){ temp = 0; };
...
Game_System.prototype.drill_SSpB_dataInit = function() {
	this._drill_SSpB_context_list = JSON.parse(JSON.stringify( DrillUp.g_SSpB_context_list ));	//深拷贝数据（杜绝引用造成的修改）
...
// * 帧刷新 - 描述图片
//==============================
Scene_Drill_SSpB.prototype.updateDescPic = function() {
	if( DrillUp.g_SSpB_descPic_showInstant == false ){
		this._window_desc_pic.opacity += 255/DrillUp.g_SSpB_descPic_slideAnim['slideTime'];
	}
}
//==============================
// * 帧刷新 - 窗口选项刷新
...
Drill_SSpB_SelectWindow.prototype.refresh = function() {
	...
	for(var i=1; i<= DrillUp.g_SSpB_context_list_length ;i++){
		...
		if( DrillUp.g_SSpB_title_data_global ){
			var temp = DrillUp.g_SSpB_context_list[i];	//全局存储
		}else{
			var temp = $gameSystem._drill_SSpB_context_list[i];	//正常存储
		}
...
Drill_SSpB_SelectWindow.prototype.drawItem = function(index) {
	...
	var xx = rect.x;
	if( DrillUp.g_SSpB_selWin_align == "居中" ){
		xx += (rect.width - str_width) * 0.5;
	}else if( DrillUp.g_SSpB_selWin_align == "右对齐" ){
