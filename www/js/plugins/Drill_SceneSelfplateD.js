...
　　var DrillUp = DrillUp || {}; 
	...
	for (var i = 1; i <= DrillUp.g_SSpD_context_list_length ; i++ ) {
		if( DrillUp.parameters['内容-' + String(i) ] != "" ){
			DrillUp.g_SSpD_context_list[i] = JSON.parse(DrillUp.parameters['内容-' + String(i)] );
		}else{
		...
		//描述内容处理
		var temp = String(DrillUp.g_SSpD_context_list[i]['描述内容']);
		temp = temp.substring(1,temp.length-1);
		temp = temp.replace(/\\\\/g,"\\");
		temp = temp.split(/\\n/);
		DrillUp.g_SSpD_context_list[i]['context'] = temp;
		DrillUp.g_SSpD_context_list[i]['contextAlign'] = String(DrillUp.g_SSpD_context_list[i]['描述内容对齐方式'] || "左对齐");
		DrillUp.g_SSpD_context_list[i]['contextAutoLineheight'] = String(DrillUp.g_SSpD_context_list[i]['描述内容是否自适应行间距'] || "true") === "true";	
		DrillUp.g_SSpD_context_list[i]['contextLineheight'] = Number(DrillUp.g_SSpD_context_list[i]['描述内容固定行间距'] || 28);
		//alert(temp);
		
		//选项名处理
		temp = String(DrillUp.g_SSpD_context_list[i]['选项名']);
		temp = temp.replace(/\\\\/g,"\\");
		DrillUp.g_SSpD_context_list[i]['name'] = temp;
		
		//显示处理
		DrillUp.g_SSpD_context_list[i]['enabled'] = (DrillUp.g_SSpD_context_list[i]['是否初始显示'] || "false") == "true" ;
		
		//锁定处理
		DrillUp.g_SSpD_context_list[i]['locked'] = (DrillUp.g_SSpD_context_list[i]['是否初始锁定'] || "false") == "true" ;
		
		//描述图片处理
		DrillUp.g_SSpD_context_list[i]['pic'] = (DrillUp.g_SSpD_context_list[i]['资源-描述图片'] || "");
		...
		if(args.length == 2){
			...
			if( type == "显示全部" ){
				for( var i = 1; i <= DrillUp.g_SSpD_context_list_length; i++){
					...
					if( !$gameSystem._drill_SSpD_context_list ){ $gameSystem.drill_SSpD_dataInit(); }
...
// * 选项窗口 - 设置选项
//==============================
Drill_SSpD_SelectWindow.prototype.initSelect = function() {
