...
Game_System.prototype.drill_SSpG_dataInit = function() {
	this._drill_SSpG_context_list = JSON.parse(JSON.stringify( DrillUp.g_SSpG_context_list ));	//深拷贝数据（杜绝引用造成的修改）
...
Scene_Drill_SSpG.prototype.drill_refreshDescPic = function( cur_index ) {
	...
	if( src_tank.length == 0 ){
		...
		for( var i=0; i < temp_list.length; i++ ){
			...
			src_tank[ i+1 ] = ImageManager.load_MenuSelfDef(DrillUp.g_SSpG_context_list[context_index]["pic"]);	
		}
...
Drill_SSpG_SelectWindow.prototype.drawItem = function(index) {
	...
	var xx = rect.x;
	if( DrillUp.g_SSpG_selWin_align == "居中" ){
		xx += (rect.width - str_width) * 0.5;
	}else if( DrillUp.g_SSpG_selWin_align == "右对齐" ){
