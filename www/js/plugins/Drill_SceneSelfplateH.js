...
　　var DrillUp = DrillUp || {}; 
	...
	DrillUp.g_SSpH_arrow_flash = String(DrillUp.parameters['是否使用闪烁效果'] || "false") === "true";	
	...
	/*-----------------流程锁定------------------*/
	DrillUp.g_SSpH_end_page = Number(DrillUp.parameters['末尾页'] || 0);
	...
	for (var i = 1; i <= DrillUp.g_SSpH_context_list_length ; i++ ) {
		if( DrillUp.parameters['内容-' + String(i) ] != "" ){
			DrillUp.g_SSpH_context_list[i] = JSON.parse(DrillUp.parameters['内容-' + String(i)] );
		}else{
		...
		// > 描述内容处理
		var temp = String(DrillUp.g_SSpH_context_list[i]['描述内容']);
		temp = temp.substring(1,temp.length-1);
		temp = temp.replace(/\\\\/g,"\\");
		temp = temp.split(/\\n/);
		DrillUp.g_SSpH_context_list[i]['context'] = temp;
		DrillUp.g_SSpH_context_list[i]['contextAlign'] = String(DrillUp.g_SSpH_context_list[i]['描述内容对齐方式'] || "左对齐");
		DrillUp.g_SSpH_context_list[i]['contextAutoLineheight'] = String(DrillUp.g_SSpH_context_list[i]['描述内容是否自适应行间距'] || "true") === "true";	
		DrillUp.g_SSpH_context_list[i]['contextLineheight'] = Number(DrillUp.g_SSpH_context_list[i]['描述内容固定行间距'] || 28);
		//alert(temp);
		
		// > 选项名处理
		temp = String(DrillUp.g_SSpH_context_list[i]['选项名']);
		temp = temp.replace(/\\\\/g,"\\");
		DrillUp.g_SSpH_context_list[i]['name'] = temp;
		
		// > 显示处理
		DrillUp.g_SSpH_context_list[i]['enabled'] = (DrillUp.g_SSpH_context_list[i]['是否初始显示'] || "false") == "true" ;
		
		// > 锁定处理
		DrillUp.g_SSpH_context_list[i]['locked'] = (DrillUp.g_SSpH_context_list[i]['是否初始锁定'] || "false") == "true" ;
		
		// > 描述图片处理
		DrillUp.g_SSpH_context_list[i]['pic'] = (DrillUp.g_SSpH_context_list[i]['资源-描述图片'] || "");
		...
		if(args.length == 2){
			...
			if( type == "显示全部" ){
				for( var i = 1; i <= DrillUp.g_SSpH_context_list_length; i++){
					...
					if( !$gameSystem._drill_SSpH_context_list ){ $gameSystem.drill_SSpH_dataInit(); }
...
// * 信息面板H - 正常存储赋值
//==============================
Game_System.prototype.drill_SSpH_dataInit = function() {
...
// * 信息面板H - 刷新箭头
//==============================
Scene_Drill_SSpH.prototype.refreshArrow = function(index) {
	//箭头显示
	var l_visible = true;
	var r_visible = true;
	var u_visible = true;
...
Drill_SSpH_SelectWindow.prototype.refresh = function() {
	...
	for( var j=0; j< $gameTemp._drill_SSpH_visibleList.length ;j++ ){
		if( $gameTemp._drill_SSpH_visibleList[j]['locked'] == false ){
...
// * 选项窗口 - 设置选项
//==============================
Drill_SSpH_SelectWindow.prototype.initSelect = function() {
...
Drill_SSpH_SelectWindow.prototype.drawItem = function(index) {
	...
	var xx = rect.x;
	if( DrillUp.g_SSpH_selWin_align == "居中" ){
		xx += (rect.width - str_width) * 0.5;
	}else if( DrillUp.g_SSpH_selWin_align == "右对齐" ){
...
// * 流程锁定 - 末尾页退出
//==============================
var _drill_SSpH_createSelect = Scene_Drill_SSpH.prototype.createSelect;
Scene_Drill_SSpH.prototype.createSelect = function() {
	_drill_SSpH_createSelect.call(this);
...
//=============================================================================
		...
		alert(
			"【Drill_SceneSelfplateH.js 面板-全自定义信息面板H】\n缺少基础插件，去看看下列插件是不是 未添加 / 被关闭 / 顺序不对："+
			"\n- Drill_CoreOfWindowAuxiliary 系统-窗口辅助核心"
		);
}
