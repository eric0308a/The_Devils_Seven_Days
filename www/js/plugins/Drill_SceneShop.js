 ...
 *   (2.窗口的布局规划没有特殊限制，你可以去看看 "窗口与布局.docx"。
 *   (3.窗口之间的流程比较复杂，建议去看看 "关于全自定义商店界面.docx"。
 *      注意，购买流程 和 出售流程 是完全固定的业务逻辑，无法修改。
 * 窗口：
 ...
 *      并且，窗口配置后的样式是固定的。
 *   (2.虽然在游戏中，结构不能临时改变，但是你可以换皮，比如使用插件
 *      指令开关菜单背景达到切换商店背景的效果，或者切换服务员。
 *   (3.更多介绍可以去看看 "关于全自定义商店界面.docx"。
 * 
 * -----------------------------------------------------------------------------
 * ----关联文件
 ...
 * 1.商店只是把 金钱 换成了 樱桃（38号物品）交易。
 *   倍率/额外价格还是原来的，你需要手动调整新的倍率/额外价格。
 * 2.注意交换商店的出售功能，你需要调整额外价格，防止玩家用这个来刷钱。
 *
 * -----------------------------------------------------------------------------
 * ----可选设定 - 服务员
 * 你可以在进入商店页面前修改服务员：
...
　　var DrillUp = DrillUp || {}; 
	...
	/*-----------------服务员参数------------------*/
	DrillUp.drill_SSh_convertWaitressAct = function( act ) {		//服务员行为数据转换
		...
		act['gif_src_file'] = "img/Menu__shop/";
	...
	for (var i = 0; i < DrillUp.g_SSh_waitress_list_length ; i++ ) {
		if( DrillUp.parameters['服务员-' + String(i+1) ] != "" ){
			...
			if( waitress_params['行为-欢迎光临'] != undefined && 
				...
				DrillUp.drill_SSh_convertWaitressAct( act );
			...
			if( waitress_params['行为-出售一个物品'] != undefined && 
				waitress_params['行为-出售一个物品'] != "" ){
				var act = JSON.parse( waitress_params['行为-出售一个物品'] || {} );
				DrillUp.drill_SSh_convertWaitressAct( act );
				waitress['act-sellOne'] = act;
			}else{
			...
			if( waitress_params['行为-余额不足(交换物)'] != undefined && 
				waitress_params['行为-余额不足(交换物)'] != "" ){
				var act = JSON.parse( waitress_params['行为-余额不足(交换物)'] || {} );
				DrillUp.drill_SSh_convertWaitressAct( act );
				waitress['act-itemNotEnough'] = act;
			}else{
				waitress['act-itemNotEnough'] = {};
			}
			DrillUp.g_SSh_waitress_list[i] = waitress;
		}else{
...
Drill_SSh_GoldWindow.prototype.refresh = function() {
	...
	this.drawCurrencyValue( this.value(), this.currencyUnit(), x, 0, width);	//货币单位，强制右对齐
};
//==============================
// * 金钱窗口 - 打开
//==============================
...
// * 金钱窗口 - 绘制货币内容
//==============================
var _drill_SSh_g_drawCurrencyValue = Drill_SSh_GoldWindow.prototype.drawCurrencyValue;
Drill_SSh_GoldWindow.prototype.drawCurrencyValue = function(value, unit, x, y, width) {
	if( $gameSystem._drill_SSh_exchange_mode ){
		// > 交换物绘制
