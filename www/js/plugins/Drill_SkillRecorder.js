 ...
 * =============================================================================
 * 一些简单物品、技能的插件指令或变量操作。
 * 该插件的逻辑会非常绕，你需要看看"关于技能逻辑关系.docx"。
 * 
 * -----------------------------------------------------------------------------
 * ----设定注意事项
 * 1.插件的作用域：战斗界面、菜单界面。
 *   只给变量作赋值，不作多余复杂操作。
 * 2.注意，记录分为 技能释放 与 物品使用 两种。
 * 3.菜单中，使用物品/释放技能 都是 我方->我方 ，
 *   所以目标敌人ID和队伍序号都会赋值0。
 *   菜单中，使用物品时默认为队长使用物品。
 *   如果有队员拥有药理知识特殊属性，则该能力最强队员会成为使用物品的人。
 * 4.如果物品/技能作用于多个对象，则赋值为最后一个作用的对象ID或者序号。
 * 5.不管 技能释放/物品使用 成功与否，数据都会被记录下来。
 *   不管 技能作用于多少单位、多少连击，记录数据都只被认定为一次技能。
 *   并且开始释放前一瞬间就会被赋值，技能/物品的公共事件可以直接取值来用。
 * 6.注意，技能中的公共事件，只能执行一次。与连击、多目标无关。
 *   但是，技能如果失败了，技能中的公共事件不会被执行。
 * 
 * -----------------------------------------------------------------------------
 * ----可选设定 - 技能使用
 ...
 *   具体还是去看看文档"关于技能逻辑关系.docx"，纯文字说明有些难理解。
 * 
 * -----------------------------------------------------------------------------
 * ----可选设定 - 物品使用
 * 如果你想节约变量的绑定，可以使用插件指令直接获取：
