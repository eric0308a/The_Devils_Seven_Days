...
//		存储数据变量	无
//		全局存储变量	DrillUp.global_TVi_visible
//		覆盖重写方法	无
//
//		工作类型		持续执行
//		时间复杂度		o(n)+o(视频图像处理) 每帧
...
//==============================
	...
	if( !DrillUp.global_TVi_visible ){	//游戏没关情况
		if( _drill_global && _drill_global[0] && _drill_global[0]["_global_TVi_visible"] ){		//游戏关闭后，重开读取global中的配置
...
// * 全局 - 存储
//==============================
var _drill_TVi_saveGlobal = DataManager.saveGlobalInfo;
DataManager.saveGlobalInfo = function(info) {	//第0个存档为全局存档
	if(!info[0]){info[0] = []};
	info[0]["_global_TVi_visible"] = DrillUp.global_TVi_visible;
	_drill_TVi_saveGlobal.call(this,info);
};
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === ">标题视频") {
		if(args.length == 4){
			...
			if( b_id >= 0 && type === "显示" ){
				DrillUp.global_TVi_visible[b_id] = true;
				DataManager.forceSaveGlobalInfo();
			}
			if( b_id >= 0 && type === "隐藏" ){
				DrillUp.global_TVi_visible[b_id] = false;
				DataManager.forceSaveGlobalInfo();
			}
		}
		if(args.length == 2){
			...
			if( type === "隐藏全部" ){
				for(var i=0; i<DrillUp.global_TVi_visible.length; i++){
					DrillUp.global_TVi_visible[i] = false;
				}
				DataManager.forceSaveGlobalInfo();
			}
		}
