 ...
 * 
 * 插件指令：change_title_window A
 *
 * 参数A：标题窗口的编号
 *
 * 示例：
 * 插件指令：change_title_window 2
 * （切换窗口布局为2号配置的图片）
 *
 * -----------------------------------------------------------------------------
 * ----更新日志
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === 'change_title_window') {
		if(args.length == 1){
			var temp = Number(args[0]) - 1;
			DrillUp.global_title_window_bitmap = DrillUp.titleWindow_list[temp];
...
/*
var _drill_Title_CommandOpen = Window_TitleCommand.prototype.open;
Window_TitleCommand.prototype.open = function() {
    if (!this.isOpen()) {	//进入游戏后，菜单就已经创建了，需要进入标题后重新刷新显现
		this._move = 0;
		this.opacity = 0;
		this.contentsOpacity = 0;
		this.x = DrillUp.titleWindow_x + DrillUp.titleWindow_slideX;
		this.y = DrillUp.titleWindow_y + DrillUp.titleWindow_slideY;
	}
    _drill_Title_CommandOpen.call(this);
};
*/
