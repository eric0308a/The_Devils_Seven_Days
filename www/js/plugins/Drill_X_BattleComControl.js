...
/*:
 * @plugindesc [v1.2]        战斗UI - 技能类型控制[扩展]
 * @author Drill_up
 *
 * @param 资源-封印攻击
 * @desc 封印攻击按钮的图片资源。
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === '>技能类型') { // >技能类型 : A : 禁用攻击
		if(args.length == 4){
			...
			if( type == '禁用攻击' ){ $gameSystem._drill_XBCC_availables[temp1][0] = true; }
			if( type == '禁用防御' ){ $gameSystem._drill_XBCC_availables[temp1][1] = true; }
			if( type == '禁用道具' ){ $gameSystem._drill_XBCC_availables[temp1][2] = true; }
			if( type == '可用攻击' ){ $gameSystem._drill_XBCC_availables[temp1][0] = false; }
			if( type == '可用防御' ){ $gameSystem._drill_XBCC_availables[temp1][1] = false; }
			if( type == '可用道具' ){ $gameSystem._drill_XBCC_availables[temp1][2] = false; }
			
			if( type == '封印攻击' ){ $gameSystem._drill_XBCC_enables[temp1][0] = true; }
			if( type == '封印防御' ){ $gameSystem._drill_XBCC_enables[temp1][1] = true; }
			if( type == '封印道具' ){ $gameSystem._drill_XBCC_enables[temp1][2] = true; }
			if( type == '解封攻击' ){ $gameSystem._drill_XBCC_enables[temp1][0] = false; }
			if( type == '解封防御' ){ $gameSystem._drill_XBCC_enables[temp1][1] = false; }
			if( type == '解封道具' ){ $gameSystem._drill_XBCC_enables[temp1][2] = false; }
		}
		if(args.length == 6){
			var temp1 = Number(args[1]);
			...
			var type = String(args[3]);
			if( type == '封印技能类' ){ 
				$gameSystem._drill_XBCC_enables[temp1][temp2 +3 -1 ] = true; 
			}
			if( type == '解封技能类' ){ 
				$gameSystem._drill_XBCC_enables[temp1][temp2 +3 -1 ] = false; 
			}
		}
	}
};
