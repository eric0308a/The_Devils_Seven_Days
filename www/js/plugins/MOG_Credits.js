 ...
 * -----------------------------------------------------------------------------
 * ----关联文件
 * 使用制作组效果，需要配置资源文件：（img/system文件夹）
 *
 * 资源-制作组背景
 * 资源-制作组内容
 *
...
Scene_MCredits.prototype.createPictureCredit = function() {
    ...
    this.pictureCredit = [];
	this.pictureCredit[0] = new Sprite(ImageManager.loadSystem( Moghunter.credits_src_CreditsA));
	this.addChild(this.pictureCredit[0]);
	if (Imported.MOG_MenuParticles && !this.skip_particles()) {this.create_mparticles()};
	this.pictureCredit[1] = new Sprite(ImageManager.loadSystem( Moghunter.credits_src_CreditsB));
