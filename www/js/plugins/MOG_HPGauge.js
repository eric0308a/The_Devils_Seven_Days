...
/*:
 * @plugindesc (v1.4)[v1.5]  战斗UI - 生命浮动框
 ...
 * 1.插件的作用域：战斗界面。
 *   放置在战斗的ui层。
 * 2.浮动框在选中时会出现（可设置），并且在玩家或敌人受伤时会出现（不可设置）。
 *
 * -----------------------------------------------------------------------------
 * ----关联文件
 * 资源路径：img/Battle__ui （Map后面有两个下划线）
 ...
 * -----------------------------------------------------------------------------
 * ----可选设定 - 一直显示
 * 你可以通过插件指令设置是否生命框一直设置：
 * 
 * 插件指令（玩家框一直显示）：  always_actorHPGauge_on
 * 插件指令（玩家框自适应显示）：always_actorHPGauge_off
 * 插件指令（敌人框一直显示）：  always_enemyHPGauge_on
 * 插件指令（敌人框自适应显示）：always_enemyHPGauge_off
 * 
 * -----------------------------------------------------------------------------
 * ----关于Drill_up优化：
 * [v1.1]
...
//插件记录：
//		主体并没有改动，都是细节的零碎优化。
//		1.最大显示位修改了（Moghunter.hp_display）
//		2.添加了插件指令（345行）
//		3.一直显示浮动框。（$gameSystem._hpGaugeEDef_alwaysActor）
//		浮动框为了适配插件指令长期存储，设置了$gameSystem。
//
...
　　var Moghunter = Moghunter || {}; 
  ...
  　Moghunter.parameters = PluginManager.parameters('MOG_HPGauge');
	...
	Moghunter.hpgauge_alwaysActor = String(Moghunter.parameters['玩家浮动框是否一直显示'] || "false") == "true"; 
	Moghunter.hpgauge_alwaysEnemy = String(Moghunter.parameters['敌人浮动框是否一直显示'] || "false") == "true"; 
...
Game_System.prototype.initialize = function() {
    ...
    this._hpGaugeEDef = [];
    this._hpGaugeEDef_alwaysActor = Moghunter.hpgauge_alwaysActor;
    this._hpGaugeEDef_alwaysEnemy = Moghunter.hpgauge_alwaysEnemy;
};
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
	...
	if (command === 'always_actorHPGauge_on') {
		$gameSystem._hpGaugeEDef_alwaysActor = true; 
	}
	if (command === 'always_actorHPGauge_off') {
		$gameSystem._hpGaugeEDef_alwaysActor = false; 
	}
	if (command === 'always_enemyHPGauge_on') {
		$gameSystem.hpgauge_alwaysEnemy = true; 
	}
	if (command === 'always_enemyHPGauge_off') {
		$gameSystem.hpgauge_alwaysEnemy = false; 
	}
}
...
HPGaugeSprite.prototype.loadBitmaps = function() {
	if (this.type === 0) {
        this._layImg = ImageManager.load_BattleUi(Moghunter.HPGaugeActor_A);
	    this._meterImg = ImageManager.load_BattleUi(Moghunter.HPGaugeActor_B);
		this._numberImg = ImageManager.load_BattleUi(Moghunter.HPGaugeActor_C);
	} else {
        this._layImg = ImageManager.load_BattleUi(Moghunter.HPGaugeEnemy_A);
	    this._meterImg = ImageManager.load_BattleUi(Moghunter.HPGaugeEnemy_B);	
		this._numberImg = ImageManager.load_BattleUi(Moghunter.HPGaugeEnemy_C);	
	};
};
...
HPGaugeSprite.prototype.forceVisible = function() {
	 ...
	 if (this.battler().isEnemy() && this.battler().isAlive() && $gameSystem._hpGaugeEDef_alwaysEnemy){return true};
	 if (this.battler().isActor() && this.battler().isAlive() && $gameSystem._hpGaugeEDef_alwaysActor){return true};
	 return false;
};
